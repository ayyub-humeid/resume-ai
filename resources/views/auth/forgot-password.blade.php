<x-layouts.app-layout pageTitle="{{ __('Forgot password') }}" pageHeading="{{ __('Reset your password') }}"
    pageDescription="{{ __('We\'ll send a secure password-reset link to your email.') }}">
    <div class="mx-auto max-w-md rounded-2xl border border-slate-700/80 bg-slate-900/70 p-6 shadow-2xl shadow-black/20 sm:p-8">
        ...
        <h2 class="mt-2 text-2xl font-bold text-white">{{ __('Forgot your password?') }}</h2>
        <p class="mt-2 text-sm leading-6 text-slate-400">{{ __('Enter the email address associated with your account and we\'ll send a reset link.') }}</p>
        @if (session('status'))
        ...
        <form method="POST" action="{{ route('password.email') }}" class="mt-6 space-y-5">
            ...
            <button type="submit" class="w-full rounded-lg bg-gradient-to-r from-blue-500 to-indigo-600 px-4 py-3 font-semibold text-white transition hover:from-blue-400 hover:to-indigo-500">{{ __('Email reset link') }}</button>
        </form>
        <p class="mt-6 text-center text-sm text-slate-400"><a href="{{ route('login') }}" class="font-semibold text-blue-400 hover:text-blue-300">{{ __('Back to sign in') }}</a></p>
    </div>
