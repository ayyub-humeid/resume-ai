<div class="space-y-8">
    <div class="grid gap-4 sm:grid-cols-3">
        <x-dashboard.stat-card label="{{ __('Active roles') }}" :value="$activeRoles" />
        <x-dashboard.stat-card label="{{ __('Candidates reviewed') }}" :value="$candidateCount" tone="indigo" />
        <x-dashboard.stat-card label="{{ __('Shortlisted') }}" :value="$shortlistedCount" tone="emerald" />
    </div>
    <div
        ...
        <div>
            <h2 class="text-xl font-bold text-white">{{ __('Your roles') }}</h2>
            <p class="mt-1 text-sm text-slate-400">{{ __('Manage the roles and candidate reviews in your pipeline.') }}</p>
        </div>
        <a href="{{ route('dashboard.recruiter.bulk-upload') }}"
            class="rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-3 text-sm font-bold text-white hover:from-blue-400 hover:to-indigo-500">{{ __('New review') }}</a>
    </div>
    ...
    <div class="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900/50 p-4 sm:grid-cols-[1fr_auto]">
        ...
        <select wire:model.live="status"
            ...
            <option value="active">{{ __('Active roles') }}</option>
            <option value="archived">{{ __('Archived roles') }}</option>
    ...
    @if ($jobs->isEmpty())
        <div class="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 px-6 py-14 text-center">
            <p class="font-semibold text-white">{{ __('No roles match your filters.') }}</p><a
                href="{{ route('dashboard.recruiter.bulk-upload') }}"
                class="mt-3 inline-block text-sm font-semibold text-blue-300 hover:text-blue-200">{{ __('Start a candidate review') }}</a>
        </div>
    @else
                ...
                <article wire:key="job-{{ $job->id }}"
                    ...
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-1 text-sm text-slate-400">{{ $job->company ?: __('Company not specified') }} ·
                                {{ $job->created_at->diffForHumans() }}</p>
                        </div>
                        <div class="flex items-center gap-3"><span
                                class="text-sm text-slate-400">{{ __(':count candidates', ['count' => $job->candidates_count]) }}</span><button
                                wire:click="toggleStatus({{ $job->id }})"
                                class="rounded-lg bg-slate-800 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-700">{{ $job->status === 'active' ? __('Archive') : __('Restore') }}</button>
                        </div>
                    </div>
                </article>
            @endforeach
