<div class="space-y-6">
    <div
        ...
        <div>
            <h2 class="text-xl font-bold text-white">Candidate rankings</h2>
            <p class="mt-1 text-sm text-slate-400">Search, filter, and move candidates through your pipeline.</p>
        </div><a href="{{ route('dashboard.recruiter.bulk-upload') }}"
            class="rounded-xl bg-blue-500 px-5 py-3 text-sm font-bold text-white hover:bg-blue-400">New review</a>
    </div>
    <div class="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900/50 p-4 md:grid-cols-3">
        <input wire:model.live.debounce.300ms="search" type="search" placeholder="Search candidates"
        ...
        <article wire:key="candidate-{{ $candidate->id }}"
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="mt-1 text-sm text-slate-400">{{ $candidate->job?->title ?: 'Unassigned role' }} ·
                        {{ $candidate->file_name }} · {{ str_replace('_', ' ', $candidate->review_state) }}</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <select wire:change="updateStatus({{ $candidate->id }}, $event.target.value)"
                        ...
                        <option value="reviewing" @selected($candidate->status === 'reviewing')>Reviewing</option>
    ...
    @empty
        <div class="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 px-6 py-14 text-center">
            <p class="font-semibold text-white">No candidates match your filters.</p>
        </div>
    @endforelse
