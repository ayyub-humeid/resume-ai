<div class="space-y-6">
    ...
    @if ($page === 'index')
        <div
            ...
            <div>
                <h2 class="text-xl font-bold text-white">{{ __('Your analyses') }}</h2>
                <p class="mt-1 text-sm text-slate-400">{{ __('Review each resume-to-role comparison in one place.') }}</p>
            </div>
            <button wire:click="create"
                class="rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-3 text-sm font-bold text-white transition hover:from-blue-400 hover:to-indigo-500">{{ __('New analysis') }}</button>
        </div>
        ...
        @if (! $hasSavedAnalyses)
            <div class="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 px-6 py-16 text-center">
                <div class="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-blue-500/15 text-2xl text-blue-300">
                    ✦</div>
                <h3 class="mt-5 text-lg font-bold text-white">{{ __('No analyses yet') }}</h3>
                <p class="mx-auto mt-2 max-w-md text-sm text-slate-400">{{ __('Choose one of your resumes and add a job description to get tailored ATS feedback.') }}</p>
                <button wire:click="create" class="mt-6 text-sm font-semibold text-blue-300 hover:text-blue-200">{{ __('Create your first analysis →') }}</button>
            </div>
        @else
            <div class="grid gap-3 rounded-2xl border border-slate-800 bg-slate-900/50 p-4 sm:grid-cols-3">
                <input wire:model.live.debounce.300ms="search" type="search" placeholder="{{ __('Search role, company, or resume') }}"
                ...
                <select wire:model.live="resumeFilter" class="rounded-xl border border-slate-700 bg-slate-950/60 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500">
                    <option value="all">{{ __('All resumes') }}</option>
                ...
                <select wire:model.live="scoreFilter" class="rounded-xl border border-slate-700 bg-slate-950/60 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500">
                    <option value="all">{{ __('All match scores') }}</option>
                    <option value="strong">{{ __('Strong match (70%+)') }}</option>
                    <option value="potential">{{ __('Potential match (45–69%)') }}</option>
                    <option value="needs-work">{{ __('Needs work (under 45%)') }}</option>
                </select>
            </div>

            @if ($analyses->isEmpty())
                <div class="rounded-2xl border border-dashed border-slate-700 bg-slate-900/40 px-6 py-12 text-center">
                    <p class="text-sm font-medium text-slate-300">{{ __('No analyses match these filters.') }}</p>
                    <button wire:click="$set('search', ''); $set('resumeFilter', 'all'); $set('scoreFilter', 'all')" class="mt-3 text-sm font-semibold text-blue-300 hover:text-blue-200">{{ __('Clear filters') }}</button>
                </div>
            @else
                        ...
                        <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-sm text-slate-400">
                                    {{ $analysis->job->company ?: __('Company not specified') }} ·
                                    {{ $analysis->resume->title }}</p>
                                <p class="mt-2 text-xs text-slate-500">{{ __('Created') }}
                                    {{ $analysis->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="flex gap-2"><button wire:click="show({{ $analysis->id }})"
                                    class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">{{ __('View') }}</button><button
                                    onclick="confirmAction('{{ __('The saved feedback and its job description will be permanently removed.') }}', () => @this.delete({{ $analysis->id }}), '{{ __('Delete analysis?') }}')"
                                    class="rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-red-500/10 hover:text-red-300"
                                    aria-label="{{ __('Delete analysis') }}">{{ __('Delete') }}</button></div>
                        </div>
                    </article>
                @endforeach
            </div>
