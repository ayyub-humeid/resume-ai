<x-layouts.dashboard-layout title="{{ __('Dashboard') }}" heading="{{ __('Good to see you, :name', ['name' => $user->name]) }}"
    description="{{ __('Keep your resumes organized and prepare for the roles you want.') }}">
    <div class="grid gap-4 sm:grid-cols-3">
        <!-- Card: Total Resumes -->
        {{-- <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm font-medium">{{ __('Total Resumes') }}</p>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm font-medium">{{ __('Analyses Performed') }}</p>
        ...
        <div class="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-slate-400 text-sm font-medium">{{ __('Average Score') }}</p>
    ...
    <!-- Quick Actions -->
    <div class="mt-8">
        <h2 class="mb-4 text-lg font-semibold text-white">{{ __('Start here') }}</h2>
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <a href="{{ route('dashboard.job-seeker.resumes.index') }}"
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-lg font-bold mb-2">{{ __('Manage Resumes') }}</h4>
                        <p class="text-blue-100 text-sm">{{ __('Add a new resume for analysis') }}</p>
                    </div>
            ...
            <a href="{{ route('dashboard.job-seeker.analyze') }}"
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-lg font-bold mb-2">{{ __('Analyze Resume') }}</h4>
                        <p class="text-purple-100 text-sm">{{ __('Get AI-powered insights') }}</p>
                    </div>
                    <svg class="w-8 h-8 opacity-50 group-hover:opacity-100 transition" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="mt-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <h2 class="text-lg font-semibold text-white">{{ __('Your next step') }}</h2>
        <div class="py-8">
            <p class="text-sm text-slate-400">{{ __('Upload a resume, then paste a job description to receive a focused match analysis.') }}</p>
        </div>
    </div>
