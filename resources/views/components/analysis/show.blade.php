...
<section class="space-y-6">
    <div
        ...
        <div><button wire:click="index" class="text-sm font-semibold text-slate-400 hover:text-white">← {{ __('All analyses') }}</button>
            <h2 class="mt-3 text-2xl font-bold text-white">{{ $analysis->job->title }}</h2>
            <p class="mt-1 text-sm text-slate-400">{{ $analysis->job->company ?: __('Company not specified') }} ·
        ...
        <div
            ...
            <div class="text-center">
                ...
                <div class="text-[10px] font-bold uppercase tracking-wider">{{ __('Match') }}</div>
            </div>
        </div>
    </div>
    <div class="grid gap-6 lg:grid-cols-2">
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <h3 class="font-bold text-white">{{ __('Matched keywords') }}</h3>
            <div class="mt-4 flex flex-wrap gap-2">
                @forelse($analysis->keywords_matched ?? [] as $keyword)
                    ...
                    <p class="text-sm text-slate-500">{{ __('No matched keywords returned.') }}</p>
                @endforelse
            </div>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <h3 class="font-bold text-white">{{ __('Missing keywords') }}</h3>
            <div class="mt-4 flex flex-wrap gap-2">
                @forelse($analysis->keywords_missing ?? [] as $keyword)
                    ...
                    <p class="text-sm text-slate-500">{{ __('No missing keywords returned.') }}</p>
                @endforelse
            </div>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <h3 class="font-bold text-white">{{ __('ATS improvements') }}</h3>
            <ul class="mt-4 space-y-3">
                ...
                @empty<p class="text-sm text-slate-500">{{ __('No issues returned.') }}</p>
                @endforelse
            </ul>
        </div>
        <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <h3 class="font-bold text-white">{{ __('Interview questions') }}</h3>
            <ol class="mt-4 space-y-3">
                ...
                <li class="text-sm leading-6 text-slate-300">{{ $loop->iteration }}. {{ $question }}</li>@empty
                    <p class="text-sm text-slate-500">{{ __('No questions returned.') }}</p>
                @endforelse
            </ol>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <div class="flex items-center justify-between">
            <h3 class="font-bold text-white">{{ __('Tailored cover letter') }}</h3>
            ...
            @if (!$showCoverLetterReview)
                <button wire:click="generateCoverLetter" wire:loading.attr="disabled" wire:target="generateCoverLetter"
                    ...
                    <span wire:loading.remove wire:target="generateCoverLetter">
                        {{ $analysis->cover_letter ? __('Regenerate cover letter') : __('Generate cover letter') }}
                    </span>
                    <span wire:loading wire:target="generateCoverLetter">{{ __('Generating…') }}</span>
                </button>
            @endif
        </div>
        ...
        @else
            <p class="mt-4 text-sm text-slate-500">{{ __('No cover letter generated yet.') }}</p>
        @endif
    </div>
</section>
