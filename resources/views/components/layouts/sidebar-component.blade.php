<aside class="hidden border-r border-slate-800 bg-slate-900/70 p-5 lg:flex lg:flex-col">
    ...
    <p class="mt-3 text-xs font-semibold uppercase tracking-[.18em] text-slate-500">
        {{ auth()->user()->isRecruiter() ? 'Recruiter workspace' : 'Candidate workspace' }}</p>
    <nav class="mt-8 space-y-1">
        @if (auth()->user()->isJobSeeker())
            <a href="{{ route('dashboard.job-seeker.index') }}"
                class="block rounded-lg px-3 py-2.5 text-sm font-medium {{ request()->routeIs('dashboard.job-seeker.index') ? 'bg-blue-500/15 text-blue-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">Overview</a>
        ...
        @else
            <a href="{{ route('dashboard.recruiter.index') }}"
                class="block rounded-lg px-3 py-2.5 text-sm font-medium {{ request()->routeIs('dashboard.recruiter.index') ? 'bg-blue-500/15 text-blue-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">Overview</a>
            <a href="{{ route('dashboard.recruiter.bulk-upload') }}"
                class="block rounded-lg px-3 py-2.5 text-sm font-medium {{ request()->routeIs('dashboard.recruiter.bulk-upload') ? 'bg-blue-500/15 text-blue-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">Bulk
                upload</a>
            <a href="{{ route('dashboard.recruiter.candidates') }}"
                class="block rounded-lg px-3 py-2.5 text-sm font-medium {{ request()->routeIs('dashboard.recruiter.candidates') ? 'bg-blue-500/15 text-blue-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">Candidates</a>
        @endif
    </nav>
    <div class="mt-auto border-t border-slate-800 pt-4">
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-3">@csrf <button
                class="w-full rounded-lg px-3 py-2 text-left text-sm text-slate-400 hover:bg-slate-800 hover:text-white">Sign
                out</button></form>
    </div>
</aside>
